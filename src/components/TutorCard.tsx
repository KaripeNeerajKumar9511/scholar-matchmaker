    ...
    <>
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            {subjects.map((subject) => (
              <Badge
                key={subject}
                variant="secondary"
                className="bg-secondary/10 text-secondary dark:bg-secondary/20 dark:text-secondary-foreground"
              >
                {subject}
              </Badge>
            ))}
      ...
      <Dialog open={showContactForm} onOpenChange={setShowContactForm}>
        <DialogContent className="sm:max-w-[425px]">
          ...
          <ContactForm tutorName={name} onClose={() => setShowContactForm(false)} />
        </DialogContent>
      </Dialog>
    </>
  );
